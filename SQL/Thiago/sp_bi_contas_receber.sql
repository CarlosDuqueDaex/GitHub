...
IF OBJECT_ID('[dbo].[sp_bi_contas_receber]') IS NOT NULL
    DROP PROCEDURE [dbo].[sp_bi_contas_receber]; 
GO

CREATE PROCEDURE [dbo].[sp_bi_contas_receber] 
...
-------------------------------------------------------------------------------------------------------------------
  ...
  SET @query= @query +
	    ...
	    '''DFcod_cliente'','+
  	    '''CLIENTE'','+
        '''DFcod_historico_padrao_receber'','+
        '''HIST_PADRAO'','+
  ...
  '   , convert(varchar(200),tb1.DFcod_historico_padrao_receber) as DFcod_historico_padrao_receber ' +
  ...
  ' from [DBDirector].[dbo].[TBtitulo_receber] as tb1 ' +
  'inner join [DBDirector].[dbo].[TBrateio_ccusto_cfinanceira_receber] as tb2  ' +
  '   on tb1.DFid_titulo_receber = tb2.DFid_titulo_receber ' +
  ...
  ' left join [DBDirector].[dbo].[TBtitulo_baixado_receber] as tb8  ' +
  '   on tb1.DFid_titulo_receber =  tb8.DFid_titulo_receber   ' +
  ' left join [DBDirector].[dbo].[TBbaixado_receber_movto_bancario] as tb9  ' +
  '   on tb8.DFid_titulo_baixado_receber = tb9.DFid_titulo_baixado_receber ' +
  ...
  'inner join [DBDirector].[dbo].[TBhistorico_padrao_receber] as tb12  ' +
  '   on tb1.DFcod_historico_padrao_receber = tb12.DFcod_historico_padrao_receber ' +
  'inner join [DBDirector].[dbo].[TBtipo_documento] as tb13  ' +
  '   on tb1.DFcod_tipo_documento = tb13.DFcod_tipo_documento'

    --print @query;
	SET @bcp_cmd4 = 'bcp "' + @query + '" queryout "\\192.168.0.6\bi\contas_receber.csv" -c -t, -T -S -C ACP';
